...
export default function PricingCheckout() {
  const handleUpgrade = (plan: string) => {
    toast.success(`Đã chọn gói dịch vụ ${plan}!`, {
      description: 'Chuyển sang cổng thanh toán an toàn...'
    });
  };

  // ponytail: PricingCheckout using Level 1 Primary Clay card for recommended plan and Level 2 Soft Clay for standard plans
  return (
    <div className="max-w-5xl mx-auto space-y-8 bg-[#F5F7FA]">
      <div className="text-center space-y-3">
        <h1 className="text-h2 font-bold text-[#0F172A]">Gói dịch vụ Chủ trọ</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch">
        ...
        <Card className="bg-white rounded-[18px] shadow-clay-soft p-8 flex flex-col justify-between space-y-6 border-none">
          ...
          <Button variant="secondary" fullWidth onClick={() => handleUpgrade('Cơ Bản')}>Đang sử dụng</Button>
        </Card>

        {/* Pro Plan (Recommended Level 1 Primary Clay) */}
        <Card className="bg-white rounded-[18px] shadow-clay-primary p-8 flex flex-col justify-between space-y-6 border-2 border-[#00153D] relative">
          ...
          <Button variant="primary" fullWidth onClick={() => handleUpgrade('Chuyên Nghiệp')}>Nâng cấp Pro ngay</Button>
        </Card>
        ...
        <Card className="bg-white rounded-[18px] shadow-clay-soft p-8 flex flex-col justify-between space-y-6 border-none">
          ...
          <Button variant="secondary" fullWidth onClick={() => handleUpgrade('Doanh Nghiệp')}>Liên hệ tư vấn</Button>
        </Card>
